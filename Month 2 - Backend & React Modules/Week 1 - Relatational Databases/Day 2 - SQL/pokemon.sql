...
INSERT INTO trainer(name, deck_id)
VALUES ('Ash', 1),
        ('Aang', 2);
...
SELECT * FROM pokemon LEFT JOIN deck ON deck.id = pokemon.deck_id RIGHT JOIN trainer ON trainer.deck_id = deck.id;

-- SELECT name, type, weight,deck_id FROM pokemon LEFT JOIN deck ON deck.id = pokemon.deck_id;
